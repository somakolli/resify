...
data class ExceptionReturn(val message: String)
...
class DataIntegrityViolation : ExceptionMapper<DataIntegrityViolationException>{
    override fun toResponse(exception: DataIntegrityViolationException?): Response =
        Response.status(409).entity(ExceptionReturn(exception!!.message!!)).build()
}
...
@Provider
class NotAuthorized: ExceptionMapper<NoAuthorizationException> {
